...
public interface MissionRepository extends JpaRepository<Mission, Long> {
    Page<Mission> findByGroupIdAndEstimateLessThanEqual(long groupId, LocalDateTime now,
            Pageable pageable);

    Page<Mission> findByGroupIdAndEstimateGreaterThan(long groupId, LocalDateTime now,
            Pageable pageable);

    List<Mission> findByGroupIdAndEstimateGreaterThanOrderByEstimate(Long id, LocalDateTime now);

    @Query(value = "SELECT group_mission.* " + "FROM group_mission, group_member "
